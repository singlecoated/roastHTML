...
roastHtmlTables <- function(x,y=NULL,mat,intvar=NULL,mc.cores.x=1,mc.cores.y=1,outdir='./',stripdir='./',stripprefix='',stripid='entrez',
                            label='MaxMean',maxgs=50,padj.th=0.10,indhtml=TRUE,returnData=TRUE,verbose=TRUE)
{
    ...
    ans <- mclapply(names(x[[1]]),function(gs) ## Roast results, iterate by geneset
    {
        ...
        {
            ans <- roastHtmlTable(x[[i]][[gs]],y[[i]],mat,intvar,gs,i,filename=sprintf('roastGSA_%s_%s_%s.html',label,gs,i),dirname=file.path(outdir,sprintf('roastGSA/html/%s/',gs)),
                                  stripdir,stripprefix,stripid,indhtml=TRUE,maxgs=maxgs,padj.th=padj.th,verbose)
            ans
        },mc.cores=mc.cores.x)
    },mc.cores=mc.cores.y)
...
roastHtmlTable <- function(x,y=NULL,mat,intvar,gs,i,filename,dirname,stripdir,stripprefix,stripid,maxgs=50,padj.th=0.10,verbose=TRUE,indhtml=TRUE)
{
    detable <- y
    ...
    if (verbose) print(sprintf('Generating Roast HTML result tables for %s / %s (%s significant genesets for padj %.2e)',gs,i,nrow(mygs$res),padj.th))
    ...
    if (indhtml) roastHtmlDETable(mygs,gs,i,detable,filename,dirname,stripdir,stripprefix,stripid,sorttable,dragtable,maxgs,verbose)
}


roastHtmlDETable <- function(mygs,gs,i,detable,filename,dirname,stripdir,stripprefix,stripid,sorttable,dragtable,maxgs,verbose=TRUE)
{
                ...
                if ((nrow(mygs$res))>0)
                    {
                        ...
                        xout[is.na(xout)] <- 'NA'
                        imgsrc <- sprintf('%s/%s.pdf',stripdir,paste0(stripprefix,xout[,stripid]))
                        xout$Plot <- sprintf('<a href="%s"><img src="%s" height=50 width=50 alt="[pdf]"></img></a>',imgsrc,imgsrc)
